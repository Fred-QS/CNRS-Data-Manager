...
<div class="cnrs-dm-front-agent-info-wrapper">
    <div class="cnrs-dm-front-agent-info">
        ...
        <div class="cnrs-dm-front-agent-info-left">
            <div class="cnrs-dm-info-avatar" style="background-image: url(<?php echo $agent['photo'] ?>);"></div>
        ...
        <div class="cnrs-dm-front-agent-info-right">
            <div class="cnrs-dm-front-agent-info-data-container">
                <h3><?php echo ucfirst($agent['prenom']) ?> <?php echo strtoupper($agent['nom']) ?></h3>
                <a href="mailto:<?php echo $agent['email_pro'] ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        <path d="M64 112c-8.8 0-16 7.2-16 16v22.1L220.5 291.7c20.7 17 50.4 17 71.1 0L464 150.1V128c0-8.8-7.2-16-16-16H64zM48 212.2V384c0 8.8 7.2 16 16 16H448c8.8 0 16-7.2 16-16V212.2L322 328.8c-38.4 31.5-93.7 31.5-132 0L48 212.2zM0 128C0 92.7 28.7 64 64 64H448c35.3 0 64 28.7 64 64V384c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V128z"/>
                    </svg>
                    <?php echo $agent['email_pro'] ?>
                </a>
                <div class="cnrs-dm-front-agent-info-data">
                    <h5><?php echo __('Status', 'cnrs-data-manager') ?></h5>
                    <span><?php echo $agent['statut'] ?><?php echo $agent['tutelle'] !== null ? ', ' . $agent['tutelle'] : '' ?></span>
                    <span><?php echo $agent['activite'] ?></span>
                    <h5><?php echo __('Membership', 'cnrs-data-manager') ?></h5>
                    <?php if (count($agent['equipes']) > 0): ?>
                        <p><?php echo count($agent['equipes']) > 1 ? __('Teams', 'cnrs-data-manager') : __('Team', 'cnrs-data-manager') ?></p>
                        <ul>
                            <?php foreach ($agent['equipes'] as $equipe): ?>
                                <li><a href="<?php echo $equipe['extra']['url'] ?>">&bull; <?php echo $equipe['extra']['title'] ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif;  ?>
                    <?php if (count($agent['services']) > 0): ?>
                        <p><?php echo count($agent['services']) > 1 ? __('Services', 'cnrs-data-manager') : __('Service', 'cnrs-data-manager') ?></p>
                        <ul>
                            <?php foreach ($agent['services'] as $service): ?>
                                <li><a href="<?php echo $service['extra']['url'] ?>">&bull; <?php echo $service['extra']['title'] ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif;  ?>
                    <?php if (count($agent['plateformes']) > 0): ?>
                        <p><?php echo count($agent['plateformes']) > 1 ? __('Platforms', 'cnrs-data-manager') : __('Platform', 'cnrs-data-manager') ?></p>
                        <ul>
                            <?php foreach ($agent['plateformes'] as $plateforme): ?>
                                <li><a href="<?php echo $plateforme['extra']['url'] ?>">&bull; <?php echo $plateforme['extra']['title'] ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif;  ?>
                    ...
                    <?php if (count($agent['liens_externes']) > 0): ?>
                        ...
                        <ul>
                            <?php foreach ($agent['liens_externes'] as $link): ?>
                                <li><a href="<?php echo $link['url'] ?>" target="_blank">&bull; <?php echo $link['nom'] ?></a></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif;  ?>
                </div>
