...
namespace CnrsDataManager\Excel\Writer\Xls;

use GdImage;
use CnrsDataManager\Excel\Cell\Cell;
use CnrsDataManager\Excel\Cell\Coordinate;
use CnrsDataManager\Excel\Cell\DataType;
use CnrsDataManager\Excel\Exception as PhpSpreadsheetException;
use CnrsDataManager\Excel\RichText\RichText;
use CnrsDataManager\Excel\RichText\Run;
use CnrsDataManager\Excel\Shared\StringHelper;
use CnrsDataManager\Excel\Shared\Xls;
use CnrsDataManager\Excel\Style\Border;
use CnrsDataManager\Excel\Style\Conditional;
use CnrsDataManager\Excel\Style\Protection;
use CnrsDataManager\Excel\Worksheet\PageSetup;
use CnrsDataManager\Excel\Worksheet\SheetView;
...
class Worksheet extends BIFFwriter
{
     ...
     * Sheet object.
     */
    public \CnrsDataManager\Excel\Worksheet\Worksheet $phpSheet;

    /**
     * Escher object corresponding to MSODRAWING.
     */
    private ?\CnrsDataManager\Excel\Shared\Escher $escher = null;
     ...
     * @param \CnrsDataManager\Excel\Worksheet\Worksheet $phpSheet The worksheet to write
     */
    public function __construct(int &$str_total, int &$str_unique, array &$str_table, array &$colors, Parser $parser, bool $preCalculateFormulas, \CnrsDataManager\Excel\Worksheet\Worksheet $phpSheet)
    {
    ...
    public function close(): void
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \CnrsDataManager\Excel\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParentOrThrow()->getDefaultStyle()->getFont());
        }
     ...
     * Get Escher object.
     */
    public function getEscher(): ?\CnrsDataManager\Excel\Shared\Escher
    {
        return $this->escher;
    }
     ...
     * Set Escher object.
     */
    public function setEscher(?\CnrsDataManager\Excel\Shared\Escher $escher): void
    {
        $this->escher = $escher;
    }
    ...
    private function writeCFRule(
        ...
        if ($bFormatFont == 1) {
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \CnrsDataManager\Excel\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= pack('C', 0x00);
                    $fontUnderline = 0;

                    break;
                case \CnrsDataManager\Excel\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= pack('C', 0x02);
                    $fontUnderline = 0;

                    break;
                case \CnrsDataManager\Excel\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= pack('C', 0x22);
                    $fontUnderline = 0;

                    break;
                case \CnrsDataManager\Excel\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= pack('C', 0x01);
                    $fontUnderline = 0;

                    break;
                case \CnrsDataManager\Excel\Style\Font::UNDERLINE_SINGLEACCOUNTING:
